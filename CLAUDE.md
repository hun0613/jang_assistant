...
9. **모든 구현 완료 후 반드시 TC(Test Case) 문서를 작성**한다
   - 위치: `docs/test/XX.md` (XX = 이슈 번호)
   - 포맷: 시나리오별 테스트 케이스 (절차 + 기대 결과 체크리스트)
   - 목적: 수동 테스트 기준 제공 및 회귀 검증용 자료 축적
10. 모든 단계 완료 시 **아래 문서들을 반드시 업데이트**한다:
...
11. 사용자에게 **push 여부를 확인** → 승인 시 push 진행
12. push 완료 후 **PR title/description을 작성**하여 제안 (PR 생성 및 머지는 사용자가 직접 수행)
